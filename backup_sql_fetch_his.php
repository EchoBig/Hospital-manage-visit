...
if ($action == 'fetch') {
	...
	$data .=' <table class="table table-bordered table-hover">
                              <thead>
                                  <tr>
                                      ...
                                      <th style="text-align: center;width: 130px;">สถานะ</th>
                                  </tr>
                              </thead>
                              <tbody>';
    if (mysqli_num_rows($query) > 0) {
    	while ($row = mysqli_fetch_array($query)) {
    		...
    		$no_status = $row['id'];
    		if ($no_status == 1) {
    			$data .= '<span class="label">'.$row['status'].'</span>';
    		}
    		elseif ($no_status == 2) {
    			$data .= '<span class="label label-success">'.$row['status'].'</span>';
    		}
    		elseif ($no_status == 3) {
    			$data .= '<span class="label label-warning">'.$row['status'].'</span>';
    		}
    		elseif ($no_status == 4) {
    			$data .= '<span class="label label-inverse">'.$row['status'].'</span>';
    		}
    		elseif ($no_status == 5) {
    			$data .= '<span class="label label-important">'.$row['status'].'</span>';
    		}
    		else {
    			$data .= '<span class="label label-info">'.$row['status'].'</span>';
    		}
    		$data .='</td>
    			</tr>';
    	}
